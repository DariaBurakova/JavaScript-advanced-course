...
class ProductsList {
    ...
    testing_name(value) {
        ...
        if (regExp_name.test(value) === false) {
            document.querySelector('.name').style.borderColor = "red"
            document.querySelector('.name').value = "Error"
        }
    }
    testing_phone(value) {
        let regExp_phone = /^7?0{10}/;
        if (regExp_phone.test(value) === false) {
            document.querySelector('.phone').style.borderColor = "red"
            document.querySelector('.phone').value = "Error"
        }
    }
    testing_mail(value) {
        let regExp_mail = /my.?-?mail@mail.ru/g;
        if (regExp_mail.test(value) === false) {
            document.querySelector('.mail').style.borderColor = "red"
            document.querySelector('.mail').value = "Error"
        }
    }
    testing_text(value) {
        let regExp_text = /[a-zа-яё0-9]/;
        if (regExp_text.test(value) === false) {
            document.querySelector('.text').style.borderColor = "red"
            document.querySelector('.text').value = "Error"
        }
    }
    init() {
        document.querySelector('.txt').addEventListener('submit', (event) => {
            event.preventDefault();
            this.testing_name(document.querySelector('.name').value);
            this.testing_phone(document.querySelector('.phone').value);
            this.testing_mail(document.querySelector('.mail').value);
            this.testing_text(document.querySelector('.text').value);
        });
